...
begin
    ...
    mixs_all = sort(map(x -> parse(Int64, split(x[findfirst("mixs=", x)[end]+1:end], "_")[1]), filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixs_simplified/exports/algonquin_vb",join=true))))
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixs_simplified/exports/algonquin_vb",join=true))
    ...
    mixs_all = sort(map(x -> parse(Int64, split(x[findfirst("mixs=", x)[end]+1:end], "_")[1]), filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixs_simplified/exports/gs_sum",join=true))))
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixs_simplified/exports/gs_sum",join=true))
        mixs = parse(Int64, split(metrics_file[findfirst("mixs=", metrics_file)[end]+1:end], "_")[1])
        mixs_ind = findall(x -> x==mixs, mixs_all)[1]
    ...
    metrics_file = "paper/experiment2_clapping_mixs_simplified/exports/wiener/metrics_power=0.h5"
...
pgfsave("paper/experiment2_clapping_mixs_simplified/exports/figures/metrics.tikz", plt_metrics)