...
begin
    ...
    mixn_all = sort(map(x -> parse(Int64, split(x[findfirst("mixn=", x)[end]+1:end], "_")[1]), filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixn/exports/algonquin_vb",join=true))))
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixn/exports/algonquin_vb",join=true))
    ...
    mixn_all = sort(map(x -> parse(Int64, split(x[findfirst("mixn=", x)[end]+1:end], "_")[1]), filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixn/exports/algonquin_vb",join=true))))
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment2_clapping_mixn/exports/gs_sum",join=true))
        mixn = parse(Int64, split(metrics_file[findfirst("mixn=", metrics_file)[end]+1:end], "_")[1])
        mixn_ind = findall(x -> x==mixn, mixn_all)[1]
    ...
    metrics_file = "paper/experiment2_clapping_mixn/exports/wiener/metrics_power=0.h5"
