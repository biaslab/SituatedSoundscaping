...
begin
    ...
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment1_clapping_SNR_simplified_optim/exports/algonquin_vb",join=true))
    ...
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment1_clapping_SNR_simplified_optim/exports/gs_sum",join=true))
        SNR = -parse(Int64, split(metrics_file[findfirst("power=", metrics_file)[end]+1:end], ".")[1])
    ...
    for metrics_file in filter(x -> occursin(".h5", x) & occursin("metrics",x), readdir("paper/experiment1_clapping_SNR_simplified_optim/exports/wiener",join=true))
        SNR = -parse(Int64, split(metrics_file[findfirst("power=", metrics_file)[end]+1:end], ".")[1])
...
pgfsave("paper/experiment1_clapping_SNR_simplified_optim/exports/figures/metrics.tikz", plt_metrics)