...
function vb_update_z!(q_μ::Array{Dnormalvector,1}, q_γ::Array{Dgammavector,1}, q_ξ::Dnormalvector, U::Array{Float64,1}, tmp::Array{Float64,1})
    U .= 0
...
# update alpha
function vb_update_qa(data::Data, q_a::Ddirichlet, v_a_down::Ddirichlet, q_μ::Array{Dnormalvector,1}, q_γ::Array{Dgammavector,1}, nr_freqs::Int64, it::Int64, observation_noise_precision::Float64)
    ...
    @showprogress "iteration "*string(it)*"a - " for n in 1:length(data)
        ...
        @inbounds for k in 1:dsize[2]
            ...
            q_ξ = Dnormalvector(d[:,k], freq1) # approx

            # update U
            vb_update_z!(q_μ, q_γ, q_ξ, U, tmp)
            # q_z = v_z_up * v_z_down
...
function vb_update_qa(data::Array{Float64,2}, q_a::Ddirichlet, v_a_down::Ddirichlet, q_μ::Array{Dnormalvector,1}, q_γ::Array{Dgammavector,1}, nr_freqs::Int64, it::Int64, observation_noise_precision::Float64)
    v_z_down = Dcategorical(exp.(logmean(q_a)) ./ sum(exp.(logmean(q_a))))
    q_a_p = deepcopy(v_a_down.a)    # clear memory and only keep prior
    m_tmp = zeros(nr_freqs)
    freq1 = 1e10*ones(nr_freqs)
    U = zeros(length(q_μ))
    tmp = zeros(nr_freqs)
...
function vb_update_μγ(data::Data, v_z_down::Dcategorical, v_μ_down::Array{Dnormalvector}, v_γ_down::Array{Dgammavector}, nr_freqs::Int64, it::Int64)
    ...
    @showprogress "iteration "*string(it)*"b - " for n in 1:length(data)
        ...
        for k in 1:dsize[2]
            ...
            # upward message from gaussianscale node toward gaussian
            v_ξ_up = Dnormalvector(d[:,k], freqh)
            q_ξ = v_ξ_up
...
function vb_update_μγ(data::Array{Float64,2}, v_z_down::Dcategorical, v_μ_down::Array{Dnormalvector}, v_γ_down::Array{Dgammavector}, nr_freqs::Int64, it::Int64)
    q_μ_new = deepcopy(v_μ_down)
    q_μ = deepcopy(v_μ_down)
    q_γ_new = deepcopy(v_γ_down)
    q_γ = deepcopy(v_γ_down)
    ...
    @showprogress "iteration "*string(it)*"b - " for n in 1:1
        ...
        for k in 1:dsize[2]
            ...
            # upward message from gaussianscale node toward gaussian
            v_ξ_up = Dnormalvector(d[:,k], freq1)
            q_ξ = v_ξ_up
...
function VB_VMP(data::Union{Data, Array{Float64,2}}, means::Array{Float64,2}, covs::Array{Float64,2}, πk::Array{Float64,1}; nr_iterations=10::Int64, observation_noise_precision=1e5::Float64)
    ...
    for it in 1:nr_iterations
        q_a = vb_update_qa(data, q_a, v_a_down, q_μ, q_γ, nr_freqs, it, observation_noise_precision)
        v_z_down = Dcategorical(exp.(logmean(q_a)) ./ sum(exp.(logmean(q_a))))
        q_μ, q_γ = vb_update_μγ(data, v_z_down, v_μ_down, v_γ_down, nr_freqs, it)
    end
...
function train_vb(model_name::String, data::Union{Data, Array{Float64,2}}, means::Array{Float64,2}, covs::Array{Float64,2}, πk::Array{Float64,1}; nr_iterations=10::Int64, observation_noise_precision=1e5::Float64, power_dB=0::Real)
    ...
    if isfile(filename)
        ...
        # initialize clusters 
        q_μ, q_γ, q_a =  VB_VMP(data, means, covs, πk; nr_iterations=nr_iterations, observation_noise_precision=observation_noise_precision)
