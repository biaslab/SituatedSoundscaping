...
export separate_sources_algonquin_vb

function separate_sources_algonquin_vb(folder, x, qs_μ, qs_γ, qs_a, qn_μ, qn_γ, qn_a; block_length::Int64=64, fs::Int64=16000, observation_noise_precision::Float64=1e5, power_dB::Real=0, save_results::Bool=true, nr_iterations::Int64=10)
    ...
    @showprogress for n in 1:nr_blocks
        ...
        G[n,:] = loop_inference_algonquin_vb(X, qs_μ, qs_γ, qs_a, qn_μ, qn_γ, qn_a, observation_noise_precision, nr_iterations)
    ...
    if save_results
        ...
        plot_algonquin_vb(folder, folder_extended, G, output, block_length, fs)
    end
...
function loop_inference_algonquin_vb(data, qs_μ, qs_γ, qs_a, qn_μ, qn_γ, qn_a, observation_noise_precision, nr_iterations)
    ...
    for (ids, idn) in Iterators.product(1:nr_mixtures_speech, 1:nr_mixtures_noise)
        ...
        # do inference
        try
            FE[ids, idn], G[ids, idn] = inference_algonquin_vb(data, qs_μ[ids], qs_γ[ids], qn_μ[idn], qn_γ[idn], observation_noise_precision, nr_iterations)
        catch e
...
function inference_algonquin_vb(data, qs_μ, qs_γ, qn_μ, qn_γ, observation_noise_precision, nr_iterations)

    # find number of frequencies
    nr_freqs = length(qs_μ)

    # create model
    model, (ξs_μ, ξs_γ, ξs, ξn_μ, ξn_γ, ξn, Y) = source_separation_model_algonquin_vb(nr_freqs, qs_μ, qs_γ, qn_μ, qn_γ, observation_noise_precision)
...
# create model
@model [ default_factorisation = MeanField() ] function source_separation_model_algonquin_vb(nr_freqs, ps_μ, ps_γ, pn_μ, pn_γ, observation_noise_precision)
...
# plot stuff
function plot_algonquin_vb(folder, folder_extended, G, output, block_length, fs)
