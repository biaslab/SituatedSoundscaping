...
# create model
@model [ default_factorisation = MeanField() ] function gaussian_mixture_model_transition(nr_samples, nr_freqs, nr_mixtures, p_μ, p_γ, p_a, observation_noise)
    ...
    # transition matrix 
    A ~ MatrixDirichlet(ones(nr_mixtures, nr_mixtures)) 
    ...
    for n in 1:nr_samples
        if n == 1
            z[n] ~ Categorical(ones(nr_mixtures)/nr_mixtures)
        else
            z[n] ~ Transition(z[n-1], A)
        end
        for freq in 1:nr_freqs
            ξ[n,freq] ~ NormalMixture(z[n], m[:,freq], w[:,freq])
            X[n,freq] ~ GS(ξ[n,freq],)
    ...
    return m, w, z, ξ, X, Y, A, scheduler
end


# specify inference procedure
function update_model_transition(model_name, data, priors, nr_files; nr_iterations=10, observation_noise=1e5)
    ...
    # else
        ...
        # create model
        model, (m, w, z, ξ, X, Y, A, scheduler) = gaussian_mixture_model_transition(nr_samples, nr_frequencies, nr_mixtures, p_μ, p_γ, p_a, observation_noise, options=(limit_stack_depth=500,));
        ...
        w_sub = subscribe!(getmarginals(w, IncludeAll()), marg_w)
        selector_sub = subscribe!(getmarginals(z, IncludeAll()), marg_selector)
        transition_sub = subscribe!(getmarginal(A, IncludeAll()), marg_A)
        
        # subscribe to free energy
        # fe_sub = subscribe!(score(Float64, BetheFreeEnergy(), model), fe)
        
        # initialize marginals
        setmarginal!(A, vague(MatrixDirichlet, nr_mixtures, nr_mixtures))
        for mix in 1: nr_mixtures
            for freq in 1:nr_frequencies
                setmarginal!(m[mix,freq], NormalMeanPrecision(p_μ[mix].μ[freq], p_μ[mix].γ[freq]))
                setmarginal!(w[mix,freq], GammaShapeRate(p_γ[mix].a[freq], p_γ[mix].b[freq]))
        ...
        for n in 1:nr_samples
            ...
            setmarginal!(z[n], Categorical(ones(nr_mixtures)/nr_mixtures))
        end
        ...
        # unsubscribe
        #unsubscribe!(fe_sub)
        unsubscribe!(transition_sub)
        unsubscribe!(selector_sub)
        unsubscribe!(m_sub)
        unsubscribe!(w_sub)
        unsubscribe!(X_sub)
        unsubscribe!(ξ_sub)
    ...
    # return extracted values
    return getvalues(marg_A)[end].data.a
