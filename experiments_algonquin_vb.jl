...
for (ndB, nmix_speech, nmix_noise) in Iterators.product(power_noise, nr_mixtures_speech, nr_mixtures_noise)
    ...
    if !overwrite 
        if isfile("exports/algonquin_vb/metrics"*id*".txt")
            continue
        end
    end
    ...
    q_μ_speech, q_γ_speech, q_a_speech = train_vb("models/VB/speech", data_speech, means_speech, covs_speech, πk2_speech; nr_iterations=nr_iterations_vb);
    ...
    q_μ_noise, q_γ_noise, q_a_noise = train_vb("models/VB/noise", log.(abs2.(recording_noise)), means_noise, covs_noise, πk2_noise; nr_iterations=nr_iterations_vb, power_dB=ndB);

    # perform source separation
    speech_sep = separate_sources_algonquin_vb("exports/algonquin_vb", mixed_signal, q_μ_speech, q_γ_speech, q_a_speech, q_μ_noise, q_γ_noise, q_a_noise; observation_noise_precision=observation_noise_precision, block_length=32, power_dB=ndB, save_results=true, nr_iterations=nr_iterations_vmp)

    # evaluate metrics
    evaluate_metrics("exports/algonquin_vb/metrics"*id*".txt", speech_sep, mixed_signal, speech_signal)
