...
function ruleVariationalARFBOutNPPP(marg_y::Nothing, 
    ...
    mθ = repeat(ForneyLab.unsafeMean(marg_θ), inner=2)
    mx = ForneyLab.unsafeMean(marg_x)
    mγ = repeat(unsafeMean(marg_γ), inner=2)
...
function ruleVariationalARFBIn1PNPP(marg_y::ProbabilityDistribution{ForneyLab.Multivariate}, 
    ...
    mθ = repeat(ForneyLab.unsafeMean(marg_θ), inner=2)
    mγ = repeat(ForneyLab.unsafeMean(marg_γ), inner=2)

    # calculate required variances
    vθ = repeat(diag(ForneyLab.unsafeCov(marg_θ)), inner=2)
...
function ruleVariationalARFBIn2PPNP(marg_y::ProbabilityDistribution{ForneyLab.Multivariate}, 
    ...
    # combine parameters by acting as if they are two colliding messages (prod!)
    xiθ = mθ[1:2:end] ./ vθ[1:2:end] + mθ[2:2:end] ./ vθ[2:2:end]
    wθ = diagm(1 ./ vθ[1:2:end] + 1 ./ vθ[2:2:end])
